...
class CounterexampleMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        if action=='walk':
            can_walk=1
            result.append((state+1,can_walk,-1))
            #result.append((state,1.-can_walk,-1))
        if action=='Tram':
            fail_prob=.5
            result.append((state*2,1.-fail_prob,-2))
            result.append((state,fail_prob,-2))
        return result
        # END_YOUR_CODE
            ...
            if state[1] is not None:
                ...
                for i,card in enumerate(self.cardValues):
                    if state[2][i]==1:
                        ...
                        prob=1/len(state[2]==1)
                        results.append((newstate,prob,-self.cardValues[i]))
        if action is "Quit":
            newstate=(state[0],None,None)
            results.append((newstate,1,0))
        if action is "Take":
            if state[1] is not None:
                val=self.cardValues[state[1]]
                state[2][state[1]]=0
                newstate=(state[0]+val,None,state[2])
                results.append((newstate,1,0))
            else:
                for i,card in enumerate(self.cardValues):
                    if state[2][i]==1:
                        prob=1/len(state[2]==1)
                        state[2][i]=0
                        val=self.cardValues[i]
                        if val+state[0]>self.threshold or len(state[2]==1)==0:
                            newstate=(state[0]+val,None,state[0]+val)
                        else:
                            newstate=(state[0]+val,None,state[2])
